...
// ---- YouTube Login via browser session + cookie extraction ----
const YOUTUBE_LOGIN_PARTITION = 'persist:mineradio-youtube-login';
const YOUTUBE_LOGIN_URL = 'https://accounts.google.com/signin/v2/identifier?service=youtube&continue=https%3A%2F%2Fwww.youtube.com';
const YOUTUBE_COOKIE_NAMES = ['SID', 'HSID', 'SSID', 'APISID', 'SAPISID', '__Secure-1PSID', '__Secure-3PSID', '__Secure-1PAPISID', '__Secure-3PAPISID', 'LOGIN_INFO', 'VISITOR_INFO1_LIVE', 'YSC', 'PREF'];

function getYoutubeCookieFile() {
  return process.env.YOUTUBE_COOKIE_FILE || path.join(app.getPath('userData'), '.youtube-cookies');
}
...
async function readYoutubeLoginCookies(cookieSession) {
  const cookies = await cookieSession.cookies.get({});
  const ytCookies = cookies.filter(c => isYoutubeCookieDomain(c.domain));
  return ytCookies;
}

function youtubeHasLogin(cookies) {
  return cookies.some(c => c.name === '__Secure-3PSID' || c.name === 'SID' || c.name === 'LOGIN_INFO');
}
...
function cookiesToNetscapeFile(cookies) {
  ...
  const rows = cookies.map(c => {
    ...
    return [domain, flag, path2, secure, expiry, c.name, c.value || ''].join('\t');
  });
...
async function openYouTubeMusicLoginWindow(owner) {
  ...
  const existingCookies = await readYoutubeLoginCookies(cookieSession);
  if (youtubeHasLogin(existingCookies)) {
    const cookieFile = getYoutubeCookieFile();
    try { fs.writeFileSync(cookieFile, cookiesToNetscapeFile(existingCookies), 'utf8'); } catch (e) { }
    const me = existingCookies.find(c => c.name === 'SID');
    return { ok: true, reused: true, cookieFile };
  }
  ...
  return new Promise((resolve) => {
    ...
    const loginWindow = new BrowserWindow({
      ...
      show: false,
      autoHideMenuBar: true,
      title: 'Đăng nhập Google / YouTube',
      backgroundColor: '#111111',
      icon: APP_ICON_ICO,
      webPreferences: {
    ...
    const checkAndSaveCookies = async () => {
      try {
        const cookies = await readYoutubeLoginCookies(cookieSession);
        if (youtubeHasLogin(cookies)) {
          const cookieFile = getYoutubeCookieFile();
          fs.writeFileSync(cookieFile, cookiesToNetscapeFile(cookies), 'utf8');
          finish({ ok: true, cookieFile });
        }
      } catch (e) {
    ...
    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      if (/^https?:\/\/([^/]+\.)?(google|youtube|googleapis|accounts\.google)\./i.test(url)) {
        loginWindow.loadURL(url).catch(e => console.warn('YouTube login nav failed:', e.message));
      } else {
        shell.openExternal(url).catch(() => { });
      }
      return { action: 'deny' };
    ...
    loginWindow.webContents.on('did-finish-load', () => checkAndSaveCookies());
    loginWindow.webContents.on('did-navigate', () => checkAndSaveCookies());

    loginWindow.on('ready-to-show', () => loginWindow.show());
    loginWindow.on('closed', async () => {
      if (settled) return;
      if (pollTimer) clearInterval(pollTimer);
      const cookies = await readYoutubeLoginCookies(cookieSession).catch(() => []);
      if (youtubeHasLogin(cookies)) {
        const cookieFile = getYoutubeCookieFile();
        try { fs.writeFileSync(cookieFile, cookiesToNetscapeFile(cookies), 'utf8'); } catch (e) { }
        resolve({ ok: true, cookieFile });
      } else {
        resolve({ ok: false, cancelled: true });
      }
    });

    pollTimer = setInterval(checkAndSaveCookies, 1500);
...
ipcMain.handle('youtube-music-get-cookie-path', async () => {
  return { path: getYoutubeCookieFile() };
});

function openSpotifyLoginWindow(owner, authUrl) {
  return new Promise((resolve) => {
    let settled = false;
    const loginWindow = new BrowserWindow({
      width: 560,
    ...
    function handleCallbackUrl(urlStr) {
      if (settled) return;
      try {
        const u = new URL(urlStr);
        if (u.pathname === '/api/spotify/auth/callback' || u.pathname === '/api/spotify/callback') {
          const code = u.searchParams.get('code') || '';
          const state = u.searchParams.get('state') || '';
          if (code) {
            settled = true;
            const redirectUriParam = u.origin + u.pathname;
            const forwardUrl = `http://127.0.0.1:${mainServerPort}/api/spotify/auth/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}&redirect_uri=${encodeURIComponent(redirectUriParam)}`;
            fetch(forwardUrl)
              .then(r => r.json())
              .then(data => {
        ...
        console.error('[SpotifyLogin] callback parse error:', e.message);
      }
    }

    loginWindow.webContents.on('will-navigate', (event, urlStr) => {
      if (urlStr.includes('/api/spotify/auth/callback') || urlStr.includes('/api/spotify/callback')) {
        event.preventDefault();
        handleCallbackUrl(urlStr);
      }
    });

    loginWindow.webContents.on('will-redirect', (event, urlStr) => {
      if (urlStr.includes('/api/spotify/auth/callback') || urlStr.includes('/api/spotify/callback')) {
        event.preventDefault();
        handleCallbackUrl(urlStr);
      }
...
ipcMain.handle('spotify-open-login', async (event, authUrl) => {
  return openSpotifyLoginWindow(getSenderWindow(event), authUrl);
});
...
async function createWindow() {
  ...
  process.env.YOUTUBE_COOKIE_FILE = getYoutubeCookieFile();
